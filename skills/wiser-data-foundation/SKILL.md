...
- Use `data.external.metadata.read` only when a source-specific readonly connection has been authorized. Request an explicit year range and bounded page; platform catalog access alone does not grant provider permission. Results are temporary metadata, not an ingested asset. Do not cache, export, add to prompts or upload those results without separate authorization; failures are not empty datasets. Default runtime wiring is disabled.
...
Provider-side self-checks may run `pnpm exec tsx scripts/data-foundation/intake-preflight.mts <original-file> <profile.json>` in the repository. The bounded JSON profile contains `declaration` and `facts` matching the discovery schemas; `facts` comes from existing parsed evidence, never model inference. This wrapper streams the original fingerprint, reuses the profile only when hash and size match, recognizes saved HTML, and calls the same pure rules as the server. Output is explicitly `PROVIDER_SELF_CHECK`; pass only its `selfCheck` reference with a later declaration, not a trusted verdict. It does not parse, upload, approve or publish.

Use `data.assessment.overview` for one target type and follow the matching resource list. Prioritize existing saved originals before acquisition, retain pending applications and retryable failures, and recheck current analysis before submitting a parse operation. The overview records next actions; it does not execute a download or auto-approve any source.

For source-backed business relations, discover `data.knowledge.relations.import/get/list/review`. Import creates pending candidates with exact saved source hashes; list defaults to approved. Keep mapping versions, source-local entity keys, multiple/contradictory evidence, reported values and missing units. Never approve as an agent or infer discharge coordinates/receiving waters from names or nearest rivers. Use the human review queue with independent authorization. Current source withdrawal invalidates reads even before the Neo4j sweep catches up.

For the Jiaming Tianjin handover, reuse `business-entities.json`, `business-relations.json` and `wastewater-monitoring-candidates.json` with `pnpm exec tsx scripts/data-foundation/prepare-tianjin-relations.mts <source-directory> <target-binding.json> <new-output-directory>`. The binding contains current target `dataItemId`, `versionId`, RAW `assetId`, `sourceHash` and an explicit `mappingVersion`; obtain these through authorized HTTP discovery/reads first. The offline preparation checks hashes declared in existing extraction results, retains all original excerpts/locations, groups evidence before making bounded import batches and never downloads, parses, ingests or approves. A preparation receipt is not proof of target ingestion. Reconcile saved originals with the target separately, then submit each batch through HTTP with retained command keys.
