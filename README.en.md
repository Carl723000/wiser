...
`/mcp` is a shared Gateway: Agent EXCON tools use the `excon_*` namespace and a RunAgent credential, while Data tools use `data_*` and a Data API identity. The two downstream identities are not interchangeable; see each MCP protocol page for discovery and invocation.
...
Or start only the process you need in separate terminals:
...
These modes do not enable unified Auth, the EXCON PostgreSQL journal, or Data Foundation. The [development guide](./apps/docs/src/content/docs/en/development/index.md) defines the exact boundary. The primary pre-commit gate is:
...
- [Databases and migrations](./apps/docs/src/content/docs/en/development/databases.md)
- [Testing and verification](./apps/docs/src/content/docs/en/development/testing.md)
