...
select is((select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
 ...
 and c.relrowsecurity and c.relforcerowsecurity), 6::bigint, 'all access tables force RLS');
...
select has_table('platform_private','project_access_requests','requests retain decisions separately from applied grants');
select has_column('platform_private','project_access_requests','requested_member_version','request execution protects intervening membership changes');
select * from finish();
rollback;
