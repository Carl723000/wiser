...
业务问题图默认分组阅读，每组显示六条真实关系。圆角文字节点按层次排列，并为卡片完整尺寸预留间距；文字保持水平。共享端点处重复的关系名称可显式开启或通过选中对象查看，独立对象对仍显示关系名称。完整网络入口与全部已授权对象的键盘查找保留。阅读控件位于独立工具栏，不遮挡画布；图与证据列表展示同一组，并分别显示当前范围与查询计数。
