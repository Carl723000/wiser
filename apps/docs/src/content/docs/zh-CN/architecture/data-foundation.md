...
业务问题图默认展示当前查询内完整的已授权关系网络，包括观测明细。选择对象或类别只筛选证据列表并突出所选对象，不删除网络节点或连线。分组阅读作为可选方式，每组显示六条准确断言。力导向、层级、环形三种布局均可按关系连通组、节点类别或对象自身的来源资料与版本组织，跨组连线保持绘制。节点间距改变斥力与节点占位、层级间隔或环半径，组间距离改变排布间隔；调节间距保留缩放，“显示全图”显式重新适配完整网络。布局及归组参数有界并保存在网址中，在同一查询的原记录与地图导航间保留，不代表证据强弱、地理距离、身份合并或正式知识审批。控件位于画布上方正常文档流，计算复用可取消 Worker，不重新请求关系。
