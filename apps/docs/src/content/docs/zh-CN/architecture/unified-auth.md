...
受管模式始终提供来源许可限制：当前资料缺少许可、许可已到期、尚未生效或已撤销时，均拒绝相应访问。只采用最新发布版本，不回退到旧版较宽许可；当前许可超过10,000项时整体拒绝，不返回截断授权。成员及委托人授权与来源许可取交集。未启用受管模式的项目沿用既有行为，迁移不自动授予来源许可或启用项目。部署须先完成控制库迁移，再更新API服务。管理端许可发布、管理范围覆盖浏览和页面接入仍需集成验收；资源包自行填写的许可说明不构成权威许可。
