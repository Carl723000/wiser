...
跨资料身份对应：IDENTITY_MATCH显式引用已存在的来源对象，不按同名合并。接收时核对名称、类型和外部标识，拒绝引用链。列表最多联合十二份明确选择的来源，每次读取重新核权；来源撤回后关联边及计数隐藏。可重建图投影使用原始端点身份。待审对应不等于已批准知识。

关系列表将`relatedSources`（最多十一份额外资料的`{dataItemId, versionId}`）及`entityReference`（`{dataItemId, versionId, mappingVersion, entityKey}`）作为JSON文本进行URL编码传递；其余查询字段保持标量。无效JSON、过大输入及不合规身份在执行前被拒绝。
