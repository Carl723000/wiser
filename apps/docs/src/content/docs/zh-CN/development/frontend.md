...
资料内容页按当前所选文件提供“资料取得与使用检查”，区分明确声明、未知项与服务器原件事实。已登录的同源 `/api/data-foundation/assessment/[action]` 经现有 DAL 调用 HTTP，限制请求体 128 KiB；结果不明确时保留重试键，原件失去访问权限时清除报告。报告按需读取并分页，初始状态不伪造检查结果。

目录按需读取取得概况，显示资料分母、检查对象选择、下一步筛选及分页资料链接，保留名称条件。查询变化时重置面板；未知不展示成质量为零或不可取得。

资料内容提供固定来源版本的业务关系折叠区。列表明确选择审核状态，仅已通过页挂载现有可访问图谱画布；对象选择同时绑定映射和来源内标识。候选文件导入核对当前资料版本，证据链接回到精确原件，审核遇到不明确失败时保留命令标识。人工审核必须填写说明，普通错误不显示上游细节。

业务关系可加入最多六十三份带版本的平台资料链接。待审图需明确选择显示并持续标明待审。节点保留来源身份或明确引用身份。每条关系显示来源、原文、记录性质、时间与位置角色及适用条件。来源空间入口不表示已有或已核验几何。

资料内容页通过严格的 `relations` JSON 参数和 `#business-relations` 锚点恢复业务图。`relation-navigation.ts` 校验资源与版本UUID、最多六十三份互异的其他来源、审核状态、待审预览选项、范围内的对象身份以及1–10页的加载边界；拒绝未知字段、重复参数和超过8192字符的载荷。参数不含证据正文或凭据。原生浏览器历史保留Next.js状态；打开或恢复历史时重新请求经过会话验证的列表，每页100条，不从链接恢复权限。后续页按关系ID累积，中断的旧恢复不能覆盖新视图。此方式不修改探索视图保存契约，也不授予项目分享权限。

关系列表与链接恢复共用契约中的63份额外来源上限；超出范围明确拒绝，不截断。恢复仍逐份核权，API每页100条与前端累计10页上限保持不变。
...
业务地图通过已授权的地图查询逐页取得完整有界要素后绘制，并沿用现有显示坐标转换。旧查询瓦片只理解来源和原有记录条件，业务地图不调用它。跨查询混页、重复、不完整或超限时显示失败，不能把部分要素冒充完整结果。

业务记录检查沿用保存的查询与审核状态，检查完整的有限关系结果，并返回选中的业务对象。文档等实体也可通过明确记录绑定回查原件。业务地图仅显示当前范围内有几何证据的记录；移动视野不筛除问题关联资料。在关系层空间语义明确之前，接口拒绝混合视口与业务条件，避免图、表、地图范围不一致。

业务对象的来源说明取自已加载、已授权的关系；来源链接按对象自身的完整引用定位，不使用提及它的关系所在资料代替。图上名称保持简短，缺失标题和同一来源内重名对象保留中性编号。
...
目录导航共用 `catalog-route.ts`：新链接使用 `version`，兼容无歧义的旧 `versionId`。重复、格式错误或互相冲突的版本参数返回错误，不退回最新版本。保存关系视图统一版本参数，在来源、返回及刷新时保持原固定版本。
