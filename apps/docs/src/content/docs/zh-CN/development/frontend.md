...
资料内容、探索中选中的资源／记录及固定版本地图复用空间来源说明组件。展开时才请求，每页读取 25 份按原文件取最新的检查；记录限定原文件，版本视图逐文件列出说明。切换选择时终止未完成请求并清空状态，范围不符或权限失效的响应不保留来源结论。折叠说明后及精细缩放时仍保留尺度限制提示；原件链接保留版本和文件编号，独立图层保留各自几何。

版本地图提供可选的单波段显示设置，包含范围图例、明确缺测码及填写的或未知的单位。应用与重试先移除旧栅格来源再终止其 Worker，保留视角、透明度和图层顺序；恢复原始配色时清除活动图例，加载错误保留到重试。两个地图页面请求俯视的连续缩放底图；高德回退为非 WebGL 模式时，业务图层限制为整数缩放并显示说明。浏览器验收须检查真实影像、两个缩放级别、透明度、缺测与零值、失败重试，以及中英文、明暗主题和 390px 布局；瓦片请求成功本身不证明位置精度。
...
空间场景请求在发起前、响应到达及响应体读取完成后检查取消状态。已放弃的请求不得通知当前场景失效或继续分页；中途分页失败不展示残缺几何，重试仍核对同一查询范围。

BusinessQuery v2允许已审与待审关系共同展示，但不改变断言状态。图谱标题和记录关系选择器使用独立的混合范围标签；记录反查逐条校验返回状态属于查询范围，保留同一查询编号。原单状态链接继续有效。

`load-exploration.server.ts`供首页与探索页共用，处理默认项目查询及显式查询、保存视图和固定版本恢复。首页运行与操作区作为服务端流式组件放在同一客户端工作区下方，状态读取失败不会隐藏可用查询。默认查询成功后，客户端沿既有探索网址规范化导航。保存专题请求推迟到展开专题入口时。

两个入口均保留流式运行与操作区，网址规范化后刷新不会丢失该区。查询失败提供明确的项目总览重开入口；空资料条件按无条件入口处理，格式错误的搜索词拒绝处理，不静默丢弃。
...
正常 `account/access` 工作区默认打开权限总览，经同源 `GET /api/platform/resources` 读取所选租户和项目的资料覆盖。服务端 DAL 将当前已验证会话传给数据基座；选择项目不授予成员身份、Data scope 或资源访问权。请求参数不能指定主体、授权快照或令牌。总量和就绪状态分布来自服务端完整查询清单，每页资料限定20条。分页沿用查询编号，筛选建立新查询。切换项目、访问被拒、查询到期及刷新失败均清除旧内容；被取消的旧请求不能重新填回资料。未知解析总量保持待统计。

资源行区分内容、空间和图谱就绪状态，并提供固定版本详情。计数解释复用支持键盘与触控的问号帮助。中英文、主题和窄屏在同一真实登录页面验收。覆盖页面本身不授予权限、不推定专业审核通过，也不据此宣称已有跨资料时空覆盖统计。

## 资源包与权限预设

已启用资源管理的项目，仅向当前项目管理员显示资源包及权限预设入口。项目响应中的可选字段 `resourceAccessEnabled` 来自实时权威状态；缺省时保留原导航。清单每页最多20项，只列各定义的最新版本。创建资源包时从当前获准的覆盖清单中明确选择资料固定版本，翻页保留已选项，相同资料与版本去重；不会自动选中未读页面，也不会授予权限。修改预设生成不可变的新版本，并提交预期旧版本以检查冲突。

同源接口在转发前核验会话、来源、有界输入和幂等键。网络结果不确定时重试沿用相同命令键；输入变化后使用新键。切换项目或权限失效会清空定义并关闭编辑区。保存回执明确区分配置已保存与成员已获权。清单读取中或拒绝访问时禁止创建。

已启用资源权限的项目为管理员及审批岗位提供“批量授权”页签。管理员只选择明确加载的成员和固定资源包/预设版本；审批岗位显示审核操作，不附带成员管理能力。申请人与接收人不显示自批操作，服务端仍独立检查。预览、批准、执行分别呈现，回执保留逐人成果；内容不变的重试沿用同一请求键与预览时间。切换项目取消旧请求；有界列表及四类办理动作均通过已核实会话的网页服务接口。

批量成员明细按操作展示固定差异；历史预览缺少快照时明确提示重新申请。授权变化须重新生成预览，历史回执不冒充当前有效访问。
