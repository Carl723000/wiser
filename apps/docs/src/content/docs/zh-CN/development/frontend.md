...
业务关系可加入最多十一份带版本的平台资料链接。待审图需明确选择显示并持续标明待审。节点保留来源身份或明确引用身份。每条关系显示来源、原文、记录性质、时间与位置角色及适用条件。来源空间入口不表示已有或已核验几何。

资料内容页通过严格的 `relations` JSON 参数和 `#business-relations` 锚点恢复业务图。`relation-navigation.ts` 校验资源与版本UUID、最多十一份互异的其他来源、审核状态、待审预览选项、范围内的对象身份以及1–10页的加载边界；拒绝未知字段、重复参数和超过8192字符的载荷。参数不含证据正文或凭据。原生浏览器历史保留Next.js状态；打开或恢复历史时重新请求经过会话验证的列表，每页100条，不从链接恢复权限。后续页按关系ID累积，中断的旧恢复不能覆盖新视图。此方式不修改探索视图保存契约，也不授予项目分享权限。
