...
const first = {
  ...
  title: '永定河', visibility: 'private', createdAt: '2026-09-18T09:00:00Z', revokedAt: null,
};
afterEach(() => { cleanup(); vi.unstubAllGlobals(); });
it('opens exact saved topics and distinguishes same titles without treating title lookup as resource search', async () => {
  const fetch = vi.fn().mockResolvedValue(Response.json({ items: [first,
    { ...first, viewId: '20000000-0000-4000-8000-000000000002', visibility: 'project' },
  ] }));
  ...
  expect(within(links[1]!.closest('li')!).getByText('有权限的项目成员')).toBeTruthy();
  await user.type(screen.getByLabelText('查找已保存专题'), '无此专题');
  expect(screen.queryByRole('link', { name: '永定河' })).toBeNull();
  expect(screen.getByText('这些已保存专题中没有匹配名称，可清除文字重新选择。')).toBeTruthy();
...
it('discards a late earlier response and clears retained topic names when access fails', async () => {
  ...
  const fetch = vi.fn().mockReturnValueOnce(new Promise<Response>(r => { resolve = r; }))
    .mockResolvedValueOnce(Response.json({ items: [{ ...first, title: '当前专题' }] }))
  ...
  resolve(Response.json({ items: [first] }));
  await waitFor(() => expect(screen.queryByRole('link', { name: '永定河' })).toBeNull());
  await user.click(screen.getByRole('button', { name: '刷新专题' }));
...
it('keeps empty and bounded topic results explicit, excluding revoked views', async () => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue(Response.json({ items: [{ ...first, revokedAt: first.createdAt }] })));
  render(<DataSavedTopics locale="en" />);
  await screen.findByText('No saved topics are available. Open data exploration to save a view.');
  expect(screen.queryByRole('link', { name: '永定河' })).toBeNull();
  expect(screen.getByText('Up to 100 authorized saved views; this list is not the complete project. Opening a topic checks source access again.')).toBeTruthy();
});
