...
// The caller supplies an independently verified fixture expectation, never a UI count.
const expectedObservationCount = Number(
  process.env['WISER_WEB_LIVE_OBSERVATION_COUNT'],
);
if (
  !Number.isSafeInteger(expectedObservationCount) ||
  expectedObservationCount < 1
)
  throw Error('A verified positive observation relation count is required');
...
test('filters real observation relations and preserves the conditions through refresh and source return', async ({
  ...
  await expect(relations.locator('article')).toHaveCount(
    expectedObservationCount,
  );
  await expect(
    relations.getByText(`筛选后关系数：${expectedObservationCount}`, {
      exact: false,
    }),
  ).toBeVisible();
  await page.reload();
