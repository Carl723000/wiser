...
it.skipIf(process.env['WISER_DATA_PG_INTEGRATION'] !== '1')(
  ...
  async () => {
    ...
    const tenantId = randomUUID(), projectId = randomUUID(), actorId = randomUUID();
    const runner = `management_api_${randomUUID().replaceAll('-', '')}`;
    const source = Array.from({ length: 6 }, () => ({
      dataItemId: randomUUID(), versionId: randomUUID(), assetId: randomUUID(),
    }));
    const context = {
      principal: {
        actorType: 'human' as const, actorId, authUserId: actorId,
        sessionId: randomUUID(), authenticationMethod: 'supabase_jwt' as const,
      },
      traceId: 'a'.repeat(32),
      authorization: {
        tenantId, projectId, purpose: 'web-console',
        maxSecurityLevel: 'L1_INTERNAL' as const,
        roles: ['source-steward'], scopes: ['platform.membership.manage'],
        authzVersion: 1,
      },
    };
    try {
      await client.query('begin');
      await client.query(`create role ${runner} nologin nosuperuser nobypassrls`);
      await client.query(`grant ${runner} to current_user with inherit false, set true`);
      await client.query(
      ...
      await client.query(`grant wiser_data_metadata to ${runner} with inherit false, set true`);
      await client.query('grant usage on schema catalog,security to wiser_data_metadata');
      ...
      for (const [index, s] of source.entries()) {
        ...
        await client.query(
          ...
          [s.dataItemId, tenantId, otherProject, `Synthetic ${index}`, published, security, accepted, authorizationScope],
        );
        await client.query(
          ...
          [s.versionId, tenantId, otherProject, s.dataItemId, published, security, accepted],
        );
      }
      ...
      const runtimePool = {
        connect: () => Promise.resolve({
          query: async (sql: string, values?: readonly unknown[]) => {
            ...
            return client.query<Record<string, unknown>>(sql, values ? [...values] : undefined);
          },
          release() {},
        }),
      };
      const permit = async () =>
        assertResourceManagementPolicy(
          {
            context,
            client: { release() {}, query: <Row>() => Promise.resolve({
              rows: [{ snapshot: {
                mode: 'managed', tenantId, projectId, actorId,
                purpose: 'web-console', now: new Date().toISOString(),
                revision: 1, grants: [], limits: [],
              } }] as Row[], rowCount: 1,
            }) },
          }, [], [],
        );
      ...
      const page = await read({
        context, page: { offset: 0, limit: 20, search: 'Synthetic' },
        signal: new AbortController().signal, managementPermit: await permit(),
      });
      expect(page.items.map((r) => r.dataItemId)).toEqual([source[0]!.dataItemId]);
      expect(page.items[0]).not.toHaveProperty('sourceContact');
      expect(page.items[0]).not.toHaveProperty('assetManifest');
      expect((await client.query<{ role: string }>('select current_user role')).rows[0]?.role).toBe(runner);
      ...
      await expect(read({
        context: wrongProject, page: { offset: 0, limit: 20, search: '' },
        signal: new AbortController().signal, managementPermit: await permit(),
      })).rejects.toThrow();
    } finally {
