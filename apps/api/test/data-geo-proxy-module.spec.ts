...
describe('Data Foundation governed GIS proxy', () => {
  it.each(['', 'amap/'])(
    'reauthorizes %s query tiles and derives all seven upstream scope values',
    async (display) => {
      const resolved = {
        ...context,
        authorization: {
      ...
      const url = `/api/data/v1/geo/tiles/vector/${display}queries/${VERSION_ID}/0/0/0.pbf`;
      ...
      expect(
        fixture.authority.authorizeExplorationQuery,
      ...
      expect(fixture.requests[0]?.path).toBe(
        `/wiser_exploration_${display ? 'amap_' : ''}mvt/0/0/0`,
      );
