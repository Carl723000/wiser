...
const RECORDS = `from catalog.analysis_record record join jsonb_array_elements($1::jsonb) ref on record.analysis_id=(ref->>'analysisId')::uuid
  where ($2::uuid is null or ref->>'versionId'=$2::text) and ($3::uuid is null or record.asset_id=$3)
    ...
    and ($8::jsonb is null or ${businessRecordPredicate('record', '$8')})`;
...
export async function queryAnalysisView(
  ...
  const selectedAssetId =
    input.view === 'records'
      ? (recordQuery?.assetId ??
        input.assetId ??
        recordAssetId ??
        businessPins?.find((pin) => pin.versionId === ref?.versionId)
          ?.assetId ??
        (spec.spatialBounds
  ...
  const filtered =
    input.view === 'records' &&
    (recordQuery || spec.spatialBounds || businessPins) &&
    selectedAssetId
